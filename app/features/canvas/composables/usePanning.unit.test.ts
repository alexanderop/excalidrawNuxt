...
const { handlers, mockUseEventListener, mockOnKeyStroke } = vi.hoisted(() => {
  ...
  const mockUseEventListener = (_target: unknown, event: string, handler: EventHandler, _opts?: unknown): void => {
    const existing = handlers.get(event) ?? []
    existing.push(handler)
    handlers.set(event, existing)
  }
  const mockOnKeyStroke = (key: string | string[], handler: EventHandler, options?: { eventName?: string; target?: unknown; dedupe?: boolean }): void => {
...
describe('usePanning', () => {
  ...
  describe('wheel events', () => {
    ...
    it('wheel event with ctrl calls zoomBy', () => {
      ...
      eventMap.fire('wheel', { deltaY: -100, ctrlKey: true, metaKey: false, offsetX: 50, offsetY: 60 })
