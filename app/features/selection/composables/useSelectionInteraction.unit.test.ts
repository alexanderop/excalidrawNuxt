...
import { getTransformHandleAtPosition } from '~/features/selection/transformHandles'
import { continueDrag } from '~/features/selection/dragElements'
import { resizeElement } from '~/features/selection/resizeElement'
...
const { handlers, mockUseEventListener } = vi.hoisted(() => {
  ...
  if (globalThis.document === undefined) {
    ...
    const noop = () => ({})
    const stubElement = { style: {}, setAttribute: noop, getAttribute: noop, addEventListener: noop, removeEventListener: noop }
...
vi.mock('~/features/selection/hitTest', () => ({
  hitTest: vi.fn(() => false),
  getElementAtPosition: vi.fn(() => null),
}))

vi.mock('~/features/selection/transformHandles', () => ({
  getTransformHandleAtPosition: vi.fn(() => null),
}))

vi.mock('~/features/selection/dragElements', () => ({
  startDrag: vi.fn(() => ({ origin: [0, 0], originalPositions: new Map() })),
  continueDrag: vi.fn(),
}))

vi.mock('~/features/selection/resizeElement', () => ({
  resizeElement: vi.fn(),
}))

vi.mock('~/features/selection/bounds', () => ({
  getElementBounds: vi.fn(() => [0, 0, 100, 100]),
}))

vi.mock('~/features/binding', () => ({
...
function createSelectionSetup() {
  ...
  const selectFn = vi.fn((id: string) => { selectedIdSet.value = new Set([id]) })
  const toggleSelectionFn = vi.fn()
  const clearSelectionFn = vi.fn(() => { selectedIdSet.value = new Set() })
  const replaceSelectionFn = vi.fn((ids: Set<string>) => { selectedIdSet.value = ids })
