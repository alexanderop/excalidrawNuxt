...
describe('hitTest', () => {
  describe('rectangle', () => {
    it('hits center of filled rectangle', () => {
      const el = createTestElement({
        type: 'rectangle', x: 10, y: 10, width: 100, height: 50, backgroundColor: '#ff0000',
      })
    ...
    it('hits interior of outline-only rectangle', () => {
      const el = createTestElement({
        type: 'rectangle', x: 10, y: 10, width: 100, height: 50, backgroundColor: 'transparent',
      })
      expect(hitTest({ x: 60, y: 35 }, el, 1)).toBe(true)
    })
    ...
    it('hits stroke of outline-only rectangle', () => {
      ...
      expect(hitTest({ x: 10, y: 35 }, el, 1)).toBe(true)
    })
    ...
    it('misses point far outside rectangle', () => {
      ...
      expect(hitTest({ x: 500, y: 500 }, el, 1)).toBe(false)
    })
  ...
  describe('ellipse', () => {
    it('hits inside filled ellipse', () => {
      const el = createTestElement({
        type: 'ellipse', x: 0, y: 0, width: 100, height: 100, backgroundColor: '#ff0000',
      })
      expect(hitTest({ x: 50, y: 50 }, el, 1)).toBe(true)
    })

    it('hits inside outline-only ellipse', () => {
      const el = createTestElement({
        type: 'ellipse', x: 0, y: 0, width: 100, height: 100, backgroundColor: 'transparent',
      })
      expect(hitTest({ x: 50, y: 50 }, el, 1)).toBe(true)
    })
    ...
    it('hits ellipse boundary within threshold', () => {
      ...
      expect(hitTest({ x: 100, y: 50 }, el, 1)).toBe(true)
    })
    ...
    it('misses corner of bounding box (outside ellipse)', () => {
      ...
      expect(hitTest({ x: 2, y: 2 }, el, 1)).toBe(false)
    })
  })
  ...
  describe('diamond', () => {
    it('hits inside filled diamond', () => {
      const el = createTestElement({
        type: 'diamond', x: 0, y: 0, width: 100, height: 100, backgroundColor: '#ff0000',
      })
      expect(hitTest({ x: 50, y: 50 }, el, 1)).toBe(true)
    })
    ...
    it('hits diamond edge within threshold', () => {
      const el = createTestElement({
        type: 'diamond', x: 0, y: 0, width: 100, height: 100, backgroundColor: 'transparent',
      })
      // Top vertex of diamond: (50, 0)
      expect(hitTest({ x: 50, y: 0 }, el, 1)).toBe(true)
    })
  })

  it('skips deleted elements', () => {
    const el = createTestElement({
      isDeleted: true, x: 0, y: 0, width: 100, height: 100, backgroundColor: '#f00',
    })
    expect(hitTest({ x: 50, y: 50 }, el, 1)).toBe(false)
  })
})
...
describe('getElementAtPosition', () => {
  it('returns topmost element when overlapping', () => {
    const bottom = createTestElement({
      id: 'a', x: 0, y: 0, width: 100, height: 100, backgroundColor: '#f00',
    })
    const top = createTestElement({
      id: 'b', x: 50, y: 50, width: 100, height: 100, backgroundColor: '#00f',
    })
