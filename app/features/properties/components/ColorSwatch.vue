...
<template>
  <UPopover v-model:open="isPickerOpen" :content="{ side: 'right', align: 'start', sideOffset: 8 }">
    <button
      :aria-label="label"
      :title="label"
    ...
    >
      ...
      <svg v-if="isTransparent" class="relative h-full w-full" viewBox="0 0 24 24">
        <line x1="4" y1="20" x2="20" y2="4" stroke="red" stroke-width="2" />
      </svg>
      <!-- Question mark for mixed -->
      <span v-else-if="isMixed" class="text-xs font-bold text-foreground/50">?</span>
      <!-- Color swatch -->
      <span v-else class="absolute inset-0.5 rounded-sm" :style="swatchStyle" />
    </button>

    <template #content>
      <div class="flex w-64 flex-col gap-2 p-3" role="dialog" aria-label="Color picker">
        <!-- Top picks row -->
        <div class="flex flex-wrap gap-1">
          <button
            ...
            :class="
              isActive(c) ? 'border-accent ring-1 ring-accent' : 'border-edge/40 hover:border-edge'
            "
            @click="selectColor(c)"
          >
            <!-- Transparent swatch -->
        ...
        <div class="grid grid-cols-10 gap-1">
          <template v-for="shade in 5" :key="`shade-${shade}`">
            <button
              ...
              :class="
                isActive(getShadeColor(name, shade))
        ...
        <!-- Recent colors -->
        <div v-if="recentColors.length > 0" class="flex flex-col gap-1">
