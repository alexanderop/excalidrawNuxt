...
vi.mock('~/features/binding/renderBindingHighlight', () => ({
...
describe('renderInteractiveScene', () => {
  ...
  it('draws selection box when provided', () => {
    ...
    const matchingFill = fillRectCalls.some(c =>
      c.args[0] === 50 && c.args[1] === 50 && c.args[2] === 200 && c.args[3] === 150,
    )
    const matchingStroke = strokeRectCalls.some(c =>
      c.args[0] === 50 && c.args[1] === 50 && c.args[2] === 200 && c.args[3] === 150,
    )
  ...
  it('renders linear editor point handles when linearEditorState provided', () => {
    ...
    const arrow = createTestArrowElement({
      ...
      x: 0,
      y: 0,
      points: [pointFrom<LocalPoint>(0, 0), pointFrom<LocalPoint>(100, 50), pointFrom<LocalPoint>(200, 0)],
    })
  ...
  it('does not draw transform handles for multiple selected elements in a group', () => {
    ...
    const el1 = createTestElement({ id: 'el-1', x: 0, y: 0, width: 50, height: 50, groupIds: [groupId] })
    const el2 = createTestElement({ id: 'el-2', x: 100, y: 0, width: 50, height: 50, groupIds: [groupId] })
