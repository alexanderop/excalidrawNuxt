...
import { distanceToShapeEdge, getHoveredElementForBinding, computeFixedPoint, getPointFromFixedPoint } from './proximity'
...
describe('distanceToShapeEdge', () => {
  describe('rectangle', () => {
    it('returns 0 for point on rectangle edge', () => {
      ...
      // Point on right edge
      const dist = distanceToShapeEdge({ x: 100, y: 25 }, el)
    ...
    it('returns correct distance for point outside rectangle', () => {
      ...
      const dist = distanceToShapeEdge({ x: 110, y: 25 }, el)
      expect(dist).toBeCloseTo(10, 5)
    })

    it('returns 0 for point on top edge', () => {
      const el = createTestElement({ x: 0, y: 0, width: 100, height: 50 })
      const dist = distanceToShapeEdge({ x: 50, y: 0 }, el)
      expect(dist).toBeCloseTo(0, 5)
    })
  })
  ...
  describe('ellipse', () => {
    it('returns 0 for point on ellipse edge', () => {
      ...
      const dist = distanceToShapeEdge({ x: 100, y: 50 }, el)
      expect(dist).toBeCloseTo(0, 5)
    })
    ...
    it('returns correct distance for point outside ellipse', () => {
      const el = createTestElement({ type: 'ellipse', x: 0, y: 0, width: 100, height: 100 })
      // 10px to the right of the right edge
      const dist = distanceToShapeEdge({ x: 110, y: 50 }, el)
      expect(dist).toBeCloseTo(10, 5)
    })
  })
  ...
  describe('diamond', () => {
    ...
    it('returns correct distance for point outside diamond', () => {
      ...
      const dist = distanceToShapeEdge({ x: 50, y: -10 }, el)
      expect(dist).toBeCloseTo(10, 5)
    })
  })

  it('handles rotated elements', () => {
    const el = createTestElement({ x: 0, y: 0, width: 100, height: 50, angle: Math.PI / 2 })
    // After 90 degree rotation, the shape's edges move
    const dist = distanceToShapeEdge({ x: 50, y: 25 }, el)
...
describe('computeFixedPoint', () => {
  it('returns [0.5, 0.5] for center of element', () => {
    ...
    const fp = computeFixedPoint({ x: 50, y: 50 }, el)
  ...
  it('returns [0, 0] for top-left corner', () => {
    const el = createTestElement({ x: 0, y: 0, width: 100, height: 100 })
    const fp = computeFixedPoint({ x: 0, y: 0 }, el)
  ...
  it('returns [1, 1] for bottom-right corner', () => {
    const el = createTestElement({ x: 0, y: 0, width: 100, height: 100 })
    const fp = computeFixedPoint({ x: 100, y: 100 }, el)
  ...
  it('clamps values to [0, 1]', () => {
    const el = createTestElement({ x: 0, y: 0, width: 100, height: 100 })
    const fp = computeFixedPoint({ x: -50, y: 200 }, el)
    expect(fp[0]).toBe(0)
    expect(fp[1]).toBe(1)
  })

  it('handles rotated elements', () => {
    const el = createTestElement({ x: 0, y: 0, width: 100, height: 100, angle: Math.PI / 4 })
    const fp = computeFixedPoint({ x: 50, y: 50 }, el)
...
describe('getHoveredElementForBinding', () => {
  ...
  it('skips deleted elements', () => {
    const rect = createTestElement({ id: 'rect1', x: 0, y: 0, width: 100, height: 50, isDeleted: true })
    const result = getHoveredElementForBinding(
      { x: 100, y: 25 },
      [rect],
      1,
  ...
  it('adjusts threshold based on zoom', () => {
    ...
    const resultZoom1 = getHoveredElementForBinding(
      { x: 110, y: 25 },
      [rect],
      1,
      new Set(),
    )
    const resultZoom5 = getHoveredElementForBinding(
      { x: 110, y: 25 },
      [rect],
      5,
      new Set(),
...
describe('computeFixedPoint and getPointFromFixedPoint round-trip', () => {
  it('round-trips for rectangle center', () => {
    ...
    const distFromCenter = Math.hypot(scenePoint.x - cx, scenePoint.y - cy)
  ...
  it('round-trips for rectangle edge point', () => {
    ...
    const fp = computeFixedPoint({ x: 100, y: 50 }, el)
    expect(fp[0]).toBeCloseTo(1, 5)
    expect(fp[1]).toBeCloseTo(0.5, 5)
    const scenePoint = getPointFromFixedPoint(fp, el)
    // Should project onto the right edge + gap
    expect(scenePoint.x).toBeCloseTo(100 + BASE_BINDING_GAP, 1)
    expect(scenePoint.y).toBeCloseTo(50, 1)
  })
})
