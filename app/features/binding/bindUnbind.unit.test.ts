...
describe('findBindableElement', () => {
  ...
  it('finds ellipse elements', () => {
    const el = createTestElement({ type: 'ellipse', id: 'ellipse1', x: 0, y: 0, width: 80, height: 60 })
    const result = findBindableElement('ellipse1', [el])
    expect(result).toBe(el)
  })

  it('finds diamond elements', () => {
    const el = createTestElement({ type: 'diamond', id: 'diamond1', x: 0, y: 0, width: 80, height: 60 })
