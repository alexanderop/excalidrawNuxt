...
export function getTransformHandles(element: ExcalidrawElement, zoom: number): TransformHandles {
  ...
  if (element.angle !== 0) {
    ...
    for (const key of ALL_HANDLE_TYPES) {
      ...
      handles[key] = [rotated[0] - handle[2] / 2, rotated[1] - handle[3] / 2, handle[2], handle[3]];
    }
  }
