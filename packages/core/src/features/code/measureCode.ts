...
export function measureCode(code: string, fontSize: number): CodeMeasurement {
  ...
  const width = Math.max(contentWidth + CODE_PADDING.left + CODE_PADDING.right, CODE_MIN_WIDTH);
  const height = Math.max(contentHeight + CODE_PADDING.top + CODE_PADDING.bottom, CODE_MIN_HEIGHT);
