import { pointFrom, pointRotateRads, radiansFrom } from '../math/index'
import type { GlobalPoint } from '../math/index'
import type { ExcalidrawElement } from '../elements/types'
import { distanceToSegment } from '../selection/hitTest'
...
export function distanceToShapeEdge(point: GlobalPoint, element: BindableElement): number {
  ...
  const unrotated = element.angle === 0
    ? point
    : pointRotateRads(point, pointFrom<GlobalPoint>(cx, cy), radiansFrom(-element.angle))
 ...
 */
export function computeFixedPoint(point: GlobalPoint, element: BindableElement): readonly [number, number] {
  const cx = element.x + element.width / 2
  const cy = element.y + element.height / 2
  const unrotated = element.angle === 0
    ? point
...
export function getPointFromFixedPoint(
  ...
  const result: GlobalPoint = edgeLen === 0
    ? edgePoint
  ...
  if (element.angle !== 0) {
    return pointRotateRads(result, pointFrom<GlobalPoint>(cx, cy), radiansFrom(element.angle))
  }
  return result
}
