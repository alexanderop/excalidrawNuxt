import type { ExcalidrawElement, ExcalidrawArrowElement, FixedPointBinding } from '../elements/types'
...
export function bindArrowToElement(
  ...
  const alreadyBound = target.boundElements.some(be => be.id === arrow.id)
  if (!alreadyBound) {
