...
export function getTransformHandles(
  ...
  if (element.angle !== 0) {
    ...
    const center = pointFrom<GlobalPoint>(elCx, elCy)
    const rads = radiansFrom(element.angle)

    for (const key of ALL_HANDLE_TYPES) {
      const handle = handles[key]
      if (!handle) continue
      const handleCenter = pointFrom<GlobalPoint>(
        handle[0] + handle[2] / 2,
        handle[1] + handle[3] / 2,
      )
      const rotated = pointRotateRads(handleCenter, center, rads)
      handles[key] = [
        rotated[0] - handle[2] / 2,
        rotated[1] - handle[3] / 2,
        handle[2],
        handle[3],
      ]
    }
...
export function getTransformHandleAtPosition(
  ...
  for (const type of ALL_HANDLE_TYPES) {
    ...
    if (
      scenePoint[0] >= hx && scenePoint[0] <= hx + hw
      && scenePoint[1] >= hy && scenePoint[1] <= hy + hh
    ) {
      return type
    }
  }
